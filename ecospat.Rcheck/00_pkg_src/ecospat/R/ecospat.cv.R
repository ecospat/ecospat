  ...
  if (jack.knife == FALSE && n.1 < cv.lim || jack.knife == FALSE && n.1 < K ||
    ...
    for (i in 1:K) {
      ...
      if (i == 1) {
        ...
        vect.predicted <- append(vect.predicted, as.double(predict(temp.glm,
          data.cv[i, ], type = "response")), after = length(vect.predicted))
      }
    ...
    df.res <- data.frame(id = df.tmp.res[, 1], obs = as.vector(glm.obj$y), predictions = df.tmp.res[,
      2])
...
# ecospat.cv.gam <- function(gam.obj, K=10, cv.lim = 10, jack.knife = FALSE)
# {
#   # CONTROL IF THE NUMBER OF OBSERVATIONS IS SUFFICIENT
...
ecospat.cv.me <- function(data.cv.me, name.sp, names.pred, K = 10, cv.lim = 10, jack.knife = FALSE) {
  ...
  # best.itr.temp <- me.perf(me.obj, method='cv', plot.it=FALSE)
  ...
  if (jack.knife == FALSE && n.1 < cv.lim || jack.knife == FALSE && n.1 < K ||
    ...
    for (i in 1:K.lst) {
      ...
      if (i == 1) {
        ...
        vect.id <- append(vect.id, j.out, after = length(vect.id))
        vect.predicted <- append(vect.predicted, as.vector(rf.tmp.val[, 2]),
          after = length(vect.predicted))
      }
...
ecospat.permut.glm <- function(glm.obj, nperm) {
  ...
  for (l in 2:l.coef) {
    v.coef[l] <- substring(names(glm.obj$coefficients[l]), regexpr(")", names(glm.obj$coefficients[l]))[1] +
      1)
  }
  ...
  resp <- attr(mterms, "response")
  vect.coeff <- c(as.vector(coef(glm.obj), mode = "numeric"), vect.dev)
  theta <- matrix(0, nperm, l.coeff, dimnames = list(NULL, names(glm.obj$coefficients)))
  dev <- matrix(0, nperm, length(dev.names), dimnames = list(NULL, dev.names))
  newframe <<- mframe
  ...
  mat.permut <- matrix(0, nperm, length(vect.names), dimnames = list(NULL, vect.names))
  ...
  for (j in 1:ncol(mat.permut)) {
    ...
    temp.val.coeff <- max(as.numeric(substring(prob.coeff, 1, nchar(prob.coeff) -
      1)))
...
ecospat.varpart <- function(model.1, model.2, model.12) {
  vect.estim <- c("D2.model.1", "D2.model.2", "D2.Joined.Contrib.1.2.", "D2.Unexplained")
  mat.estim <- matrix(0, length(vect.estim), 1, dimnames = list(vect.estim, "Deviance"))
  ...
  # common to 1 and 2, note that it could be negative (ie no 'common' part, 1 and 2
  # is larger than 1+2, see Hamilton 87 for linear regression)
