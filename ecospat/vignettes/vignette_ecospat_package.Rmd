...
```
Niche equivalency test H1: the observed overlap between the native and invaded niche is higher than if the two niches are randomized
...
sim.test <- ecospat.niche.similarity.test(grid.clim.nat, grid.clim.inv,
                                          ...
                                          rand.type=2) 
```
Niche similarity test H1: the observed overlap between the native and invaded is higher than randomly shifted invasive niches in the invaded study area
...
invisible(capture.output(ecospat.CommunityEval (eval, pred, proba=T, ntir=5)))
```
...
# remove insivible(capture.output)) to print output in the console
# this is just to keep the vignette short
invisible(capture.output(ecospat.SESAM.prr(proba, sr)))
```
