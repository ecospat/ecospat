...
## MEVA.TABLE ##
################
# Function originally from A. Guisan (Unil-ECOSPAT, Switzerland)
# Modified from L. Maiorano (Unil-ECOSPAT, Switzerland)
ecospat.meva.table <- function(Pred, Sp.occ, th)
# Pred: vector of predicted probabilities
# Sp.occ: vector of binary observations
...
#  eva <- list()	
		...
		nmi <- (- (a*log(a)) - (b*log(b)) - (c*log(c)) - (d*log(d)) + ((a+b)*log(a+b)) + ((c+d)*log(c+d))) / ((N*log(N)) - ((a+c)*log(a+c)) - ((b+d)*log(b+d)))
		tss <- se + sp -1
		mtrcNAME <- c("Prevalence", "Correct classification rate", "Mis-classification rate", "Sensitivity", "Specificity", "Positive predictive power", "Negative predictive power", "False positive rate", "False negative rate", "Odds Ratio", "Kappa", "Normalized mutual information", "True skill statistic")
		mtrcVALUE <- c(round(prev, digits=4), round(ccr, digits=4), round(mcr, digits=4), round(se, digits=4), round(sp, digits=4), round(fpr, digits=4), round(fnr, digits=4), round(ppp, digits=4), round(npp, digits=4), round(or, digits=4), round(kappa, digits=4), round(nmi, digits=4), round(tss, digits=4))
		mtrcMATR <- matrix(c(mtrcNAME, mtrcVALUE), nrow=length(mtrcVALUE), ncol=2, dimnames=list(c(1:13), c("Metric", "Value")))
...
# Function originally from A. Guisan (Unil-ECOSPAT, Switzerland)
# Modified from L. Maiorano (Unil-ECOSPAT, Switzerland)
ecospat.max.kappa <- function(Pred, Sp.occ)
# Pred: vector of predicted probabilities
# Sp.occ: vector of binary observations
{
	k <- 0.01
...
## MAX-TSS ##
###############
# Function originally from L. Maiorano (Unil-ECOSPAT, Switzerland)
# modyfying MAX-KAPPA from A. Guisan (Unil-ECOSPAT, Switzerland)
ecospat.max.tss <- function(Pred, Sp.occ)
# Pred: vector of predicted probabilities
# Sp.occ: vector of binary observations
{
	tss <- 0.01
...
##################
# Function originally from L. Maiorano (Unil-ECOSPAT, Switzerland)
ecospat.plot.kappa <- function(Pred, Sp.occ)
# Pred: vector of predicted probabilities
# Sp.occ: vector of binary observations
{
	k <- 0.01
...
##################
# Function originally from L. Maiorano (Unil-ECOSPAT, Switzerland)
ecospat.plot.tss <- function(Pred, Sp.occ)
# Pred: vector of predicted probabilities
# Sp.occ: vector of binary observations
{
	tss <- 0.01
