...
ecospat.CCV.communityEvaluation.bin <- function(ccv.modeling.data,
  ...
  for(th in thresholds){
    for(r in 1:dim(ccv.modeling.data$DataSplitTable)[2]){
      ...
      #c.m.c.s<-assign("community.metrics.cali.selected",NULL, envir = .GlobalEnv)
      #c.m.e.s<-assign("community.metrics.eval.selected",NULL, envir = .GlobalEnv)
      
      #Parsing the results
      #ccv.metrics.allsites.cali[which(ccv.modeling.data$DataSplitTable[,r]),th,,r] <- community.metrics.cali.selected[1:length(which(ccv.modeling.data$DataSplitTable[,r])),]
      #ccv.metrics.allsites.cali[which(ccv.modeling.data$DataSplitTable[,r]),th,,r] <- c.m.c.s[1:length(which(ccv.modeling.data$DataSplitTable[,r])),]
      ...
      #ccv.metrics.allsites.eval[which(!ccv.modeling.data$DataSplitTable[,r]),th,,r] <- community.metrics.eval.selected[1:length(which(!ccv.modeling.data$DataSplitTable[,r])),]
      #ccv.metrics.allsites.eval[which(!ccv.modeling.data$DataSplitTable[,r]),th,,r] <- c.m.e.s[1:length(which(!ccv.modeling.data$DataSplitTable[,r])),]
